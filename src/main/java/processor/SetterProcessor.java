...
@SupportedAnnotationTypes(value = {"annotation.NewSetter"})
public class SetterProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        Set<? extends Element> elementsAnnotatedWith = roundEnv.getElementsAnnotatedWith(NewSetter.class);
    ...
    private void addImportInfo(Element element) {
        ...
        if (treePath.getCompilationUnit() instanceof JCTree.JCCompilationUnit && leaf instanceof JCTree) {
            ...
            for (JCTree jcTree : jccu.getImports()) {
                if (jcTree instanceof JCTree.JCImport) {
                    ...
                    if (jcImport.qualid instanceof JCTree.JCFieldAccess) {
                        ...
                        try {
                            if ("package".equals(jcFieldAccess.selected.toString()) && "class".equals(jcFieldAccess.name.toString())) {
                                return;
                            }
            ...
            JCTree.JCIdent ident = treeMaker.Ident(names.fromString("package"));
            JCTree.JCImport jcImport = treeMaker.Import(treeMaker.Select(
                    ident, names.fromString("class")), false);
